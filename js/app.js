...
async function search() {
    ...
    try {
        ...
        // 创建一个队列来控制并发请求数
        const queue = [];
        const maxConcurrent = AGGREGATED_SEARCH_CONFIG.maxConcurrentRequests;
        let allResults = [];
        
        // 处理单个API源的搜索
        async function handleApiSearch(apiId) {
            let retryCount = 0;
            while (retryCount < AGGREGATED_SEARCH_CONFIG.retryTimes) {
                try {
                    ...
                    const pageCount = Math.min(
                        data.pagecount || 1,
                        AGGREGATED_SEARCH_CONFIG.maxPages
                    );
                            ...
                            pagePromises.push((async () => {
                                try {
                                    ...
                                    const pageTimeoutId = setTimeout(
                                        ...
                                        AGGREGATED_SEARCH_CONFIG.pageTimeout
                                    );
                                    ...
                                    console.warn(`分页请求失败: ${apiName} 第${page}页`, error);
                                    return null;
                                }
                            })());
                        }
                    ...
                    retryCount++;
                    if (retryCount >= AGGREGATED_SEARCH_CONFIG.retryTimes) {
                        console.warn(`API ${apiId} 搜索失败:`, error);
                        return null;
                    }
                    await new Promise(resolve => setTimeout(resolve, AGGREGATED_SEARCH_CONFIG.retryDelay));
                }
            }
            return null;
        }
        ...
        for (let i = 0; i < selectedAPIs.length; i += maxConcurrent) {
            ...
            const batchPromises = batch.map(apiId => handleApiSearch(apiId));
            ...
            // 如果已经有足够的结果，可以提前结束搜索
            if (allResults.length >= AGGREGATED_SEARCH_CONFIG.maxResults) {
                allResults = allResults.slice(0, AGGREGATED_SEARCH_CONFIG.maxResults);
                break;
            }
        }
        ...
        if (yellowFilterEnabled) {
            const banned = ['伦理片','理论片','韩国伦理','日本伦理','福利','里番动漫','门事件','萝莉少女','制服诱惑','国产传媒','cosplay','黑丝诱惑','无码','日本无码','有码','日本有码','SWAG','网红主播', '色情片','同性片','福利视频','福利片'];
